# FlowPro
A modern plumbing company website with real-time scheduling system, service booking, and stunning UI/UX design. Featured portfolio piece showcasing Vue.js, Nuxt 3, Tailwind 4, and Firebase.

## 🚀 Features
- **Real-Time Scheduling**: Interactive calendar showing plumber availability with drag-and-drop booking
- **Emergency Services**: 24/7 emergency booking with priority pricing and quick access
- **Service Catalog**: Comprehensive plumbing services with cost estimator tool
- **Portfolio Gallery**: Before/after photo showcase with customer testimonials
- **Mobile-First Design**: Optimized for emergency bookings on any device
- **Modern UI/UX**: Smooth animations with Vue 3 composables and Tailwind 4

## 🛠️ Tech Stack
- **Frontend**: Vue 3, Nuxt 3, JavaScript
- **Styling**: Tailwind CSS 4 with custom design system
- **Animations**: Vue 3 Transitions & Framer Motion for polished interactions
- **Calendar**: Vue Calendar component for scheduling
- **Backend**: Firebase (Firestore, Real-time Database, Auth, Storage)
- **Deployment**: Vercel with performance optimization

## 📱 Responsive Design
- Mobile-optimized calendar and booking flow
- Touch-friendly interactions for emergency situations
- Progressive Web App capabilities
- Fast loading on cellular connections

## 🎯 Business Goals
- Streamline appointment booking process
- Showcase professional work quality
- Generate emergency service leads
- Build customer trust with transparency

## 🚀 Getting Started
```bash
npm install
npm run dev
```

## 📊 Performance Targets
- Lighthouse score: 95+
- Load time: <3 seconds on 3G
- Mobile optimization: 100%
- SEO score: 100

---

**FlowPro** - Professional plumbing services with modern convenience